...
- **Body Content:** 
  - **Dynamic Hook:** A brief, region-specific opening sentence generated via LLM based on current local fashion trends to capture attention.
...
3. **Product Matching & API Hydration:** Users in Group A are matched with 4 relevant product IDs via a BigQuery script that ranks active inventory based on the user's `reselling_platform` and `user_intent` preferences using a nearest-neighbor similarity score. The script then hits the internal Fleek Product API (e.g., `GET /api/v1/products?ids=...`) to "hydrate" those raw IDs with live image URLs, product names, and pricing, ensuring no out-of-stock items are pushed.
4. **Dynamic Localization & News Injection:** The script fetches the latest country-specific news headlines regarding fashion, sustainability, vintage clothing, or Vinted selling. An LLM step generates a brief, localized introductory hook and translates the email copy into the user's native language. To prevent A/B test contamination, this LLM generation happens strictly at the *region level* (e.g., one standard hook for all UK users today), rather than generating uniquely unvetted copy for every single user, ensuring content variation remains minimal.
...
### Causal Inference & Measurement Challenges
In CRM and lifecycle marketing, it is notoriously easy to confuse correlation (e.g., "users who open emails tend to buy more") with causation ("the email *caused* them to buy more"). This tracking architecture explicitly solves for causal inference through strict Randomized Controlled Trial (RCT) principles, while proactively mitigating practical measurement challenges:
...
4. **Holdout Contamination Risk:** A critical measurement challenge is that users in the Global Control might still receive paid retargeting ads (e.g., Meta or Google app-install campaigns). If the performance marketing team ramps up paid spend targeting these unonboarded users during the test, the baseline conversion rate of the Global Control will artificially inflate, shrinking the apparent incremental lift of our emails. We cannot enforce a strict suppression list across all paid channels for users enrolled in this experiment to prevent cross-channel contamination, but we should keep this in mind when analyzing the results, if possible removing those who recieved paid retargeting ads. 
5. **Cross-Channel Attribution Conflicts:** When users finally install the app, their device-level attribution (via AppsFlyer) might credit a recent Facebook ad click rather than our email push. By prioritizing server-side tracking using our own internal `crm_activation_log` (which relies on `user_id` deterministic matching rather than probabilistic device fingerprinting), we ensure the A/B test evaluates the true omni-channel impact of the CRM activation, bypassing superficial "last click" attribution wars in third-party analytics platforms.
6. **LLM Content Variance:** To maintain the integrity of the A/B test, we must ensure that the dynamically generated LLM news hooks do not become confounding variables. By generating these hooks strictly at the *region level* (rather than per individual user), the variation is minimized and evenly distributed across all cohorts within that region, preserving our ability to measure the isolated impact of the £20 incentive and product catalog.
